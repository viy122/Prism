...
<style>
    ...
    .po-no-results { display: none; flex-direction: column; align-items: center; justify-content: center; gap: 10px; min-height: 180px; border-radius: 12px; border: 1.5px dashed var(--s300); background: var(--s50); padding: 32px; text-align: center; }
    .po-no-results.visible { display: flex; }
    .po-no-results i { font-size: 34px; color: var(--s300); }
    .po-no-results p { font-size: 13px; color: var(--s400); max-width: 260px; line-height: 1.6; }
...
<div class="content">
    ...
    <div class="po-grid">
        ...
        <div class="card" style="padding-bottom: 22px;">
            ...
            @else
            <div class="po-no-results" id="poNoResults">
                <i class="ti ti-search-off"></i>
                <p>No purchase orders match your search or filters.</p>
            </div>
            <div class="table-wrap" id="poTableWrap">
                <table>
                    <thead>
                        <tr>
                            <th>PO No.</th>
...
(function () {
    ...
    const poNoResults    = document.getElementById('poNoResults');
    ...
    function applyPoSearchFilter() {
        ...
        getRows().forEach(row => {
            ...
            if (match) visible++;
            // PO No., AOC, Office, Supplier — the columns data-search is
            // built from that also render as plain text.
        ...
        if (poNoResults) poNoResults.classList.toggle('visible', visible === 0);
        if (poTableWrap) poTableWrap.style.display = visible === 0 ? 'none' : '';
    }
