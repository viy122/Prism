...
@section('content')
    <section class="mb-5 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_22px_rgba(15,23,42,0.045)] lg:flex-row lg:items-start lg:justify-between [&_h1]:mt-1.5 [&_h1]:text-2xl [&_h1]:font-extrabold [&_h1]:tracking-tight [&_h1]:text-slate-950 lg:[&_h1]:text-[1.8rem]">
        <div>
            ...
            <p class="mt-2 max-w-3xl text-base leading-7 text-slate-600">Review quarterly procurement accomplishment, completed purchases, and delayed items with reasons or remarks.</p>
        </div>
        <button class="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-bsu-maroon px-4 text-sm font-bold text-white shadow-sm shadow-bsu-maroon/15 transition hover:bg-bsu-maroon-900 focus:outline-none focus:ring-2 focus:ring-bsu-gold/70" type="button" id="printProcurementReportButton">
            <i data-lucide="printer" aria-hidden="true"></i>
            Export to PDF or Print
        </button>
    </section>

    <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Procurement report summary">
        <article class="{{ $statCardClass }}">
            <span>Items targeted</span>
            <strong>{{ number_format($totalTargeted) }}</strong>
            <small>Total procurement targets in the current report set</small>
        </article>
        <article class="{{ $statCardClass }}">
            <span>Items procured</span>
            <strong>{{ number_format($totalProcured) }}</strong>
            <small>Completed against quarterly targets</small>
        </article>
        <article class="{{ $statCardClass }}">
            <span>Completion rate</span>
            <strong>{{ $averageCompletion }}%</strong>
            <small>Overall targeted-versus-procured accomplishment</small>
        </article>
        <article class="{{ $statCardClass }}">
            <span>Delayed items</span>
            <strong>{{ count($delayedItems) }}</strong>
            <small>Items requiring procurement follow-up</small>
        </article>
    </section>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_22px_rgba(15,23,42,0.05)]">
        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between [&_h2]:mt-1.5 [&_h2]:text-lg [&_h2]:font-extrabold [&_h2]:tracking-tight [&_h2]:text-slate-950">
            <div>
                <p class="text-xs font-extrabold uppercase tracking-[0.14em] text-bsu-maroon">Quarterly accomplishment</p>
                <h2>Items Targeted vs Procured</h2>
            </div>
            <span class="inline-flex min-h-7 items-center rounded-full bg-bsu-maroon/5 px-3 py-1 text-xs font-bold text-bsu-maroon ring-1 ring-inset ring-bsu-maroon/10">{{ $averageCompletion }}% completion</span>
        </div>
    ...
    <section class="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2">
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_22px_rgba(15,23,42,0.05)]">
            <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between [&_h2]:mt-1.5 [&_h2]:text-lg [&_h2]:font-extrabold [&_h2]:tracking-tight [&_h2]:text-slate-950">
                <div>
                    <p class="text-xs font-extrabold uppercase tracking-[0.14em] text-bsu-maroon">Completed purchases</p>
                    <h2>Completed This Quarter</h2>
                </div>
            </div>
            <div class="max-h-[64vh] overflow-auto rounded-2xl border border-slate-200 bg-white shadow-inner shadow-slate-100">
                <table class="min-w-full border-separate border-spacing-0 text-left text-sm text-slate-700 [&_th]:sticky [&_th]:top-0 [&_th]:z-10 [&_th]:border-b [&_th]:border-slate-200 [&_th]:bg-slate-50 [&_th]:px-4 [&_th]:py-3 [&_th]:text-xs [&_th]:font-extrabold [&_th]:uppercase [&_th]:tracking-[0.08em] [&_th]:text-slate-500 [&_td]:border-b [&_td]:border-slate-100 [&_td]:px-4 [&_td]:py-3.5 [&_td]:align-top [&_tbody_tr]:transition [&_tbody_tr:hover]:bg-bsu-maroon/5">
                    ...
                    <tbody>
                        @foreach ($completedPurchases as $purchase)
                            <tr>
                                ...
                                <td>{{ $purchase['item'] }}</td>
                                <td>{{ $purchase['prNumber'] }}</td>
                                <td>{{ $purchase['completedDate'] }}</td>
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_22px_rgba(15,23,42,0.05)]">
            <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between [&_h2]:mt-1.5 [&_h2]:text-lg [&_h2]:font-extrabold [&_h2]:tracking-tight [&_h2]:text-slate-950">
                <div>
                    <p class="text-xs font-extrabold uppercase tracking-[0.14em] text-bsu-maroon">Delayed items</p>
                    <h2>Reasons and Remarks</h2>
                </div>
            </div>
                ...
                @foreach ($delayedItems as $item)
                    <div class="flex flex-col gap-3 rounded-2xl border border-red-200 bg-red-50/70 p-4 transition hover:bg-white hover:shadow-sm sm:flex-row sm:items-start sm:justify-between [&_strong]:block [&_strong]:text-sm [&_strong]:font-bold [&_strong]:text-slate-950 [&_span]:mt-1 [&_span]:block [&_span]:text-sm [&_span]:leading-6 [&_span]:text-slate-600">
                        <div>
                            ...
                            <span>{{ $item['office'] }} - {{ $item['prNumber'] }}</span>
                            <span>{{ $item['reason'] }}</span>
                        </div>
                        <x-prism.status-badge status="Delayed" />
                    </div>
                @endforeach
            </div>
        </article>
